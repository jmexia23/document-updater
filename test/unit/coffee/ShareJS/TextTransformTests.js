...
describe("ShareJS text type", function() {
	...
	describe("transform", function() {
		...
		return describe("comment / comment", () =>
			it("should not do anything", function() {
				const dest = [];
	...
	describe("apply", function() {
		...
		it("should throw an error when deleted content does not match", () =>
			(() => text.apply("foo123bar", [{ d: "456", p: 3 }])).should.throw(Error)
		);
		
		return it("should throw an error when comment content does not match", () =>
			(() => text.apply("foo123bar", [{ c: "456", p: 3 }])).should.throw(Error)
		);
	});
	
	return describe("applying ops and comments in different orders", () =>
		it("should not matter which op or comment is applied first", function() {
			let length, p;
			let asc, end;
			let asc1, end1;
			let asc3, end3;
